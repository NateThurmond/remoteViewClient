(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("remoteView", [], factory);
	else if(typeof exports === 'object')
		exports["remoteView"] = factory();
	else
		root["remoteView"] = factory();
...
var _rvDomDiff = __webpack_require__(48);

var _rvDomElem = __webpack_require__(49);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// Our servers
var supportServer = 'http://nate.sytes.net:3001';

var clientServer = 'http://nate.sytes.net:3000';
...
var rvIsMobile = _helper2.default.detectMobile();
...
function onResize() {
  if (!supportUser) {
    socket.compress(true).emit('resize', { 'windowInnerWidth': window.innerWidth,
      ...
      'windowOuterHeight': window.outerHeight, 'htmlWidth': (0, _jquery2.default)('html').width(),
      'htmlHeight': (0, _jquery2.default)('html').height() });
  }
}
...
function mouseMovementReceive(data) {
  ...
  _rvDomElem.rvDomElem.moveSupportCursor(parseInt(data.x1, 10) + parseInt(data.w, 10) - parseInt(window.pageXOffset, 10), parseInt(data.y1, 10) + parseInt(data.h, 10) - parseInt(window.pageYOffset, 10));
}
...
function mouseMovementReceiveCanvas(data) {
  ...
  if (data.mouseDown && !supportUser) {
    ...
    var calcXCanvas = parseInt(data.x1, 10) + parseInt(data.w, 10) - parseInt(window.pageXOffset, 10);

    var calcYCanvas = parseInt(data.y1, 10) + parseInt(data.h, 10) - parseInt(window.pageYOffset, 10);

    // Briefly hide the mouse cursor to pick up element underneath
    _rvDomElem.rvDomElem.toggleSupportCursor(false);

    // Determine what element the user is scrolling on
    var elemRemoteScroll = document.elementFromPoint(parseInt(calcXCanvas, 10), parseInt(calcYCanvas, 10));

    // Reshow support cursor now that we have determined the element underneath
    _rvDomElem.rvDomElem.toggleSupportCursor(true);
    ...
    if (elemRemoteScroll !== null && elemRemoteScroll.nodeName === 'CANVAS') {
      ...
      // Generate js event
      var myCanvasEvent = document.createEvent('Event');
...
function remoteViewDisconnect() {
  ...
  _helper2.default.eraseCookie('remoteViewCookie');

  // Disconnect client end of socket
  socket.disconnect();

  // Set support button and cursor to default (disabled and hidden)
  _rvDomElem.rvDomElem.toggleSupportButton(false);
  _rvDomElem.rvDomElem.toggleSupportCursor(false);
  ...
  document.removeEventListener('mousemove', _helper2.default.throttle(onMouseMove, 10), { passive: false });
  ...
  document.removeEventListener('touchend', onMouseUp, { passive: false });
  document.removeEventListener('touchmove', _helper2.default.throttle(onMouseMove, 10), { passive: false });
  window.removeEventListener('resize', _helper2.default.throttleLastCall(onResize, 1000), true);
...
function remoteViewDomUpdate(updatedDom) {
  if (!_jquery2.default.isEmptyObject(updatedDom) && socket !== null) {
    if (supportUser) {
      ...
      socket.compress(true).emit('domUpdate', { 'domUpdate': updatedDom });
    }
  }
}
...
function remoteViewConnect() {
  ...
  if (!supportUser) {
    _helper2.default.setCookie('remoteViewCookie', clientId, 1);
  }
  ...
  var handShakeData = supportUser ? { query: 'supportUser_Id=' + supportUserId + '&endUserId=' + passedClientId + '&reconnect=' + reconnectSupport } : { query: 'clientId=' + clientId + '&documentUrl=' + encodeURIComponent(document.URL) + '&redirectClientView=' + redirectClientView };

  // Create socket and send initial data
  socket = (0, _socket2.default)(selectedServer, handShakeData);
  ...
  document.addEventListener('mousemove', _helper2.default.throttle(onMouseMove, 10), { passive: false });
  document.addEventListener('touchstart', onMouseDown, { passive: false });
  document.addEventListener('touchend', onMouseUp, { passive: false });
  document.addEventListener('touchend', onMouseUp, { passive: false });
  document.addEventListener('touchmove', _helper2.default.throttle(onMouseMove, 10), { passive: false });
  window.addEventListener('resize', _helper2.default.throttleLastCall(onResize, 1000), true);
  ...
  if (!supportUser) {
    domDiffTimer = setInterval(function () {
      ...
      _rvDomDiff.rvDomDiff.getDomDiff(false, function (vDom) {
  ...
  socket.on('leftClick', function (clickCoord) {
    ...
    var calcX = parseInt(clickCoord.x, 10) + parseInt(clickCoord.w, 10) - parseInt(window.pageXOffset, 10);

    var calcY = parseInt(clickCoord.y, 10) + parseInt(clickCoord.h, 10) - parseInt(window.pageYOffset, 10);

    // Determine what element is under the click
    var elemRemoteClick = document.elementFromPoint(parseInt(calcX, 10), parseInt(calcY, 10));
    ...
    _rvDomElem.rvDomElem.tiltSupportCursor();
  });

  // When client resizes window, adjust support view to same size
  socket.on('resize', function (windowDim) {
    (0, _jquery2.default)('html').css('width', windowDim.windowInnerWidth + 'px');
    (0, _jquery2.default)('html').css('height', windowDim.windowInnerHeight + 'px');
  });

  // When receiving dom updates pass to function to apply them
  socket.on('domUpdate', _rvDomDiff.rvDomDiff.domUpdate);
}
...
function supportButtonClick(e) {
  ...
  if (supportUser) {
    ...
    _rvDomElem.rvDomElem.removeSupportButton();
    _rvDomElem.rvDomElem.removeSupportCursor();
  } else {
    // if ($('#remoteViewSupportRequestButton').html() === 'Disable Remote View') {
    if (_rvDomElem.rvDomElem.supportButtonActive()) {
      remoteViewDisconnect();
      ...
      _rvDomElem.rvDomElem.toggleSupportButton(true);
    }
  }
};
...
function startApp() {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    supportUserId = _helper2.default.getUrlParameter('supportUser_Id');
    passedClientId = _helper2.default.getUrlParameter('endUser_Id');
    reconnectSupport = _helper2.default.getUrlParameter('reconnect');
    supportUser = _jquery2.default.inArray(supportUserId, ['', 'null', null]) === -1 && passedClientId !== '';

    // Create reference to document body
    _rvDomElem.rvDomElem.docBody((0, _jquery2.default)('body'));

    // Create the support button and cursor
    _rvDomElem.rvDomElem.createSupportButton();
    _rvDomElem.rvDomElem.createSupportCursor();
    ...
    if (supportUser) {
      ...
      var prevCookie = _helper2.default.getCookie('remoteViewCookie');
    ...
    setTimeout(function () {
      _rvDomDiff.rvDomDiff.getDomDiff(true, function (justStoreResp) {
        console.log(justStoreResp);
      });
    }, 1000);

    // Add buttons and cursors to dom needed for remote support
    _rvDomElem.rvDomElem.appendSupportButton();
    _rvDomElem.rvDomElem.appendSupportCursor();

    // Add listener for support button click
    (0, _jquery2.default)('#' + _rvDomElem.rvDomElem.supportButtonId()).on('mousedown', supportButtonClick);
...
var Helper = function () {
  ...
  _createClass(Helper, [{
    ...
    value: function setCookie(name, value, days) {
      ...
      var remoteViewDomain = window.location.hostname;
      ...
      document.cookie = name + '=' + (value || '') + expires + '; domain=' + remoteViewDomain + '; path=/';
    }
  }, {
    key: 'getAllCookies',
    value: function getAllCookies() {
      ...
      for (var i = 1; i <= allCookies.length; i++) {
        var fullCookie = allCookies[i - 1].trim();
    ...
    key: 'throttleLastCall',
    value: function throttleLastCall(callback, delay) {
...
var RvDomDiff = function () {
  ...
  _createClass(RvDomDiff, [{
    ...
    key: 'updateDomOnClick',
    value: function updateDomOnClick() {
    ...
    key: 'domUpdate',
    value: function domUpdate(deliveredDomUpdate) {
    ...
    key: 'parseDomUpdates',
    value: function parseDomUpdates() {
      ...
      for (var domUpdateInd in obj.deliveredDoms) {

        // Actual dom update
        var domUpdateParsed = obj.deliveredDoms[domUpdateInd]['domUpdate'];
        ...
        try {
          ...
          obj.updateElement((0, _jquery2.default)('body'), domUpdateParsed, prevDom, 0, true);
    ...
    key: 'listIterate',
    value: function listIterate(elem, diffObj, cb) {
      var obj = this;
      ...
      diffObj['data'] = (0, _jquery2.default)(elem).data();

      if (_jquery2.default.inArray((0, _jquery2.default)(elem).val(), ['undefined']) === -1) {
        diffObj['attrs']['val'] = (0, _jquery2.default)(elem).val();
      }
      ...
      if (diffObj['type'] === 'canvas') {
        ...
        var canvasElemId = elem.attr('id');

        var canvasElemClass = elem.attr('class');

        var cWidth = elem[0].width;

        var cHeight = elem[0].height;
      ...
      (0, _jquery2.default)(elem).each(function () {
        _jquery2.default.each(this.attributes, function () {
          if (this.specified) {
            diffObj['attrs'][this.name] = this.value;

            if (this.name === 'type' && this.value === 'checkbox') {
              diffObj['attrs']['checked'] = (0, _jquery2.default)(elem).prop('checked');
            }
          }
        });
      });
      ...
      elem.contents().each(function () {
        ...
        var foundNodeType = (0, _jquery2.default)(this)[0].nodeType;

        var foundNodeName = (0, _jquery2.default)(this)[0].nodeName;

        if (_jquery2.default.inArray(foundNodeType, [8, 4, 7, 10, 12]) !== -1) {
          ...
          diffObj['children'][childCounter] = { 'type': '#text', 'data': null, 'attrs': null,
            'children': [(0, _jquery2.default)(this)[0].data] };
        } else {
          diffObj['children'][childCounter] = { 'type': null, 'data': null, 'attrs': null, 'children': [] };
          obj.listIterate((0, _jquery2.default)(this), diffObj['children'][childCounter], cb);
        }
    ...
    value: function vDom(cb) {
      ...
      var bodyClone = (0, _jquery2.default)('body').clone(true, true).off();

      // Special case for selects, clone does not copy these values for selects
      var selects = (0, _jquery2.default)('body').find('select');

      (0, _jquery2.default)(selects).each(function (i) {
        var select = this;

        bodyClone.find('select').eq(i).val((0, _jquery2.default)(select).val());
      });
      ...
      var vDomObj = { 'type': null, 'data': null, 'attrs': null, 'children': [] };
    ...
    value: function createElement(node) {
      ...
      if (typeof node.attrs !== 'undefined') {
        obj.applyAttr((0, _jquery2.default)(el), node.attrs);
      }
      if (typeof node.data !== 'undefined') {
        obj.applyData((0, _jquery2.default)(el), node.data);
      }
    ...
    key: 'dataChanged',
    value: function dataChanged(node1, node2) {
      ...
      if (_typeof(node1.type !== 'undefined') && node1.type === 'canvas' || _typeof(node2.type !== 'undefined') && node2.type === 'canvas') {
        return true;
      }
    ...
    value: function updateElement(parent, newNode, oldNode) {
      ...
      if (!initialCall) {
        if (_jquery2.default.inArray(foundNodeType, [8, 4, 7, 10, 12]) !== -1 || foundNodeType === 3 && foundNodeName === '#text') {
      ...
      if (initialCall) {
        ...
        if (obj.attrChanged(newNode, oldNode)) {
          obj.applyAttr((0, _jquery2.default)('body'), newNode.attrs);
        }

        if (obj.dataChanged(newNode, oldNode)) {
          obj.applyData((0, _jquery2.default)('body'), newNode.data);
        }
        ...
        for (var i = 0; i < newLength || i < oldLength; i++) {
          obj.updateElement((0, _jquery2.default)('body'), newNode.children[i], oldNode.children[i], i);
        }
      } else if (!oldNode) {
        parent.append((0, _jquery2.default)(obj.createElement(newNode)));
      } else if (!newNode) {
        ...
        parent.contents().eq(index).replaceWith((0, _jquery2.default)(obj.createElement(newNode)));
    ...
    key: 'getDomDiff',
    value: function getDomDiff(justStore, callback) {
...
var rvDomDiff = exports.rvDomDiff = new RvDomDiff();
...
var _image = __webpack_require__(50);
...
var RvDomElem = function () {
  ...
  _createClass(RvDomElem, [{
    key: 'supportButtonId',
    value: function supportButtonId() {
      return this._supportButtonId;
    }
  }, {
    key: 'supportCursorId',
    value: function supportCursorId() {
      return this._supportCursorId;
    }
  }, {
    key: 'supportButtonActive',
    value: function supportButtonActive() {
      return (0, _jquery2.default)('#' + this.supportButtonId()).html() === this.rvSupportButtonTexts[1];
    }
  }, {
    key: 'docBody',
    value: function docBody(bodyElem) {
    ...
    key: 'createSupportButton',
    value: function createSupportButton() {
      this.rvSupportButton = document.createElement('p');
      this.rvSupportButton.id = this.supportButtonId();
      this.rvSupportButton.style.cssText = 'background-color: blue; width: 148px; position: ' + 'absolute; bottom: 0; right: 0; z-index: 2147483647;';
    ...
    key: 'createSupportCursor',
    value: function createSupportCursor() {
      this.rvSupportCursor = document.createElement('img');
      this.rvSupportCursor.id = this.supportCursorId();
      this.rvSupportCursor.style.cssText = 'display: none; position: fixed; z-index: ' + '2147483647; width: 30px; height: 30px;';
      this.rvSupportCursor.src = _image2.default.defaultCursor();
    }

    // Toggle support button text

  }, {
    key: 'toggleSupportButton',
    value: function toggleSupportButton() {
      var onOffBool = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
      ...
      if (onOffBool === '') {
        ...
        (0, _jquery2.default)('#' + this.supportButtonId()).html(this.rvSupportButtonTexts[nextTextInd]);
      } else {
        ...
        (0, _jquery2.default)('#' + this.supportButtonId()).html(supportButtonHtml);
      }
    }
    ...
    key: 'toggleSupportCursor',
    value: function toggleSupportCursor() {
      var onOffBool = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
      ...
      if (onOffBool === '') {
        ...
        (0, _jquery2.default)('#' + this.supportCursorId()).css('display', this.rvSupportCursorDisplays[nextDisplayInd]);
      } else {
        ...
        (0, _jquery2.default)('#' + this.supportCursorId()).css('display', supportCursorDisplay);
      }
    }
    ...
    key: 'tiltSupportCursor',
    value: function tiltSupportCursor() {
      var _this = this;
      ...
      (0, _jquery2.default)('#' + this.supportCursorId()).css('transform', rvDegrees);
      setTimeout(function () {
        (0, _jquery2.default)('#' + _this.supportCursorId()).css('transform', 'rotate(0deg)');
      }, 500);
    }
    ...
    key: 'moveSupportCursor',
    value: function moveSupportCursor(x, y) {
      this.rvSupportCursor.style.cssText = this.rvSupportCursor.style.cssText.replace('display: none;', 'display: block;');

      if (this.rvSupportCursor.style.cssText.indexOf('left') !== -1) {
        this.rvSupportCursor.style.cssText = this.rvSupportCursor.style.cssText.replace(/left:.*px;/, 'left: ' + x + 'px;');
        this.rvSupportCursor.style.cssText = this.rvSupportCursor.style.cssText.replace(/top:.*px;/, 'top: ' + y + 'px;');
      } else {
      ...
      (0, _jquery2.default)('#' + this.supportCursorId()).css({ 'display': 'block', 'left': x + 'px', 'top': y + 'px' });
    }

    // Append support button

  }, {
    key: 'appendSupportButton',
    value: function appendSupportButton() {
    ...
    key: 'appendSupportCursor',
    value: function appendSupportCursor() {
    ...
    value: function removeSupportButton() {
      (0, _jquery2.default)('#' + this.supportButtonId()).remove();
    }
    ...
    value: function removeSupportCursor() {
      (0, _jquery2.default)('#' + this.supportCursorId()).remove();
    }
  }, {
    key: 'name',
    get: function get() {
...
var rvDomElem = exports.rvDomElem = new RvDomElem();
...
var Helper = function () {
  ...
  _createClass(Helper, [{
    ...
    value: function defaultCursor() {
      ...
      return 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMgAAADICAQAAAAHUWYV' + 'AAAHL0lEQVR42u2daWwVVRiGX25baEuBKwIBJGVpSgNlrbgUCDHgD1xQIyhCKhoEN' + 'AZiIpEQFg1VwAQjmIAgSFyiRgzgghq1SotBEBERDfJDJCYQ7aUCLS1rl/EHUM43vb' + 'MU7pmZzLzP/Oucuefc7+ncOefMnG8AQgghhBBCCCGEEEIIIYQQQgghhBBCCCGEEEI' + 'IIYQQQgghQWEElmMnqtCIBiRQgSUYxKD4xVj8CCPJVo7bGRyvycKGpDIubU1YiQwG' + 'yTtuwG4bHZe2MrRnoLwhE7scdRgw8DXSGSwvWOtKhwEDSxks/Yw2Bf0o5qI/0pCOg' + 'ViAhNhXj8EMmG6+FyF/z3SliOMzsf9jBkwvg0W4P0CbFiXS8KVSohG9GDSdLFGC/S' + '86JC3TFTVKqWcYNJ2UK6FeYFnqFaXUFgZNJ5VKqK0nScYopf5g0HTSoITaeizeTSl' + 'VxaDpI00JdINNuRylXC3Dpo/0axBSx7BRCIVQCIUQCqEQQiEUQiiEQgiFUAiFUAih' + 'EAohFEIhhEIohFAIhVAIhVAIhVAIoRAKIRRCIYRCKCS5kBhDFxwhTaiBgSacwG68i' + 'rGU47cQ8/YXZiKNoQyOEAMG9mEggxkkIQZqcQ/DGSQhBi7ibgY0SEIM1DX/cMWSJC' + 'AgWoSUowS5iCENeZiJn01KDmAd9uM0DBg4iT1YhTvZC9MnpAr3ttjzOM44nDd/42m' + 'mddIhJIGCpPuKHZUY+A1DGexUCxlnuXe6i+vLWUxkuFMp5FPbz/nFhZIGPMSAp07I' + 'BNvPme2qF3YORQAyoneZH4YXUYEEGtGASlSgFEOuU0h7xG1rHNL8Kdube2H9MAN7T' + 'EpOXU709B8qsBj9oiBjDH5I+t9ZgeLrEOI0V9XFshdWglrLM6YRm9A7zDIysa6VuX' + 'fdCnEiDgOV6J903y2XxyVWUy6Tw6oj7iL37jfI1iSkCXdY7p1i26amcOaoa+cy9+4' + '2cUlNnRD7PHO7HJRMCp+Q113n3p2vRch42/1PiHoaW7SpBj3DpWOU6QtWYj4KkYEM' + 'FGKBSONn4AJGpFxIJ+TY7s+HAQPVeAEFaIM0DMYKnBOteitcQsrFl9uMTqb/X5l79' + '0jz/lQJceqF5cDAIVOPahCOiRTN3cOjo9CkI5ake7vdVCa1Qpz7f9XIbfHXobig1D' + '8nPEIWK18rYTo7rpCLU0LJbI+FLHK88oUo/WyZ8rWetyz1gBByHkWeCsmzGMaGMv2' + 's+ls8zKbca0LJYXT0TEi6xR1ENf3s8fAIUfsr2Tbl2mKvULLJMyF2I/wrW3X0hAB9' + 'US2UzPFZSKdwCjmqfK0ih7ITTWOS4AipCedF/SXH0qstbyBRSIpYJO443OQ467WPQ' + 'vRSIAL7neMzUXnifSB+CukYTiHAVyK0zzmWfzggQjoo9Z8Ok5Ai1ItL9XDHI9YGQk' + 'iI37qwUAT3ELIcryT7AyYkZCuzYqa3ra1xPCLfdGuVQlKMefrQeXnAFArRy2QR4AS' + '6OR6xnkL08o5Q8rlj+Uwc8FVI+7AL6YAjQslTLsYwtQERcgahZKR479oZi2elVEp8' + 'FJIdfiFAqThH9rpYm7ExEELOhlVIuumJ2lLHI7LwO4XoJB91ovdU7HjEANRRiE6eN' + 'N2szXE84jEK0Yt8EmuDiyPepBCddENCKLnPxZWEQrQywTRu7xrANmZFSQiwQSj5JO' + 'BCzoVfSA4OCyXTA9fCzGgJAYrFuP00+gSsffHovRZ8qThHdgRsBWxvcZWLBBmmNeR' + 'zA9W6m5WWHUREKBRPNp5HYYDaViLWP0aGuabsbxmBadmyVt10Dg0x7GjldKNXqM8B' + 'zEKE6CseaKhXVhn6SbZ4tngYIsUMcY4cRLuAzSWcil4GlC+EkhUBaNG7Sns+QuTog' + 'RMiz8go30fp6lqVaYggU8U58qcv87tXmSYefo0jkmwWSlb62padAZ/49ISuOC5+tk' + 'YHYoxu4H5Elkmmn61sn9rxvtKKY9HOHf+hULLKlzb0EcsnFiLS3CiS0TRipA9tWCf' + 'uFHZBxHnQtJbE60FiL5yP5hyWNZuEkmUe175WTOP0oQ6gi+ht1btYApfK68fF8ObJ' + 'unbkWpL9HmZnf1v8K+RRxRW2CiXzPKp1gLjLv5EartIdJ0Vfx5u0xpvFhElvalCRS' + 'fTLPKhxOJqUGldTgZlvhZJHtde3TSzQ6UEBZvqJd38kNM+5yvmr5Qx/MuaJIOn9Ed' + 'kqMpt0ZvCTkY5fxURKkbaaCkQC5SUMvRW3iUDt1lbPGnF+xBl4a94QP1uPaKkjRzz' + '38jKDbkdnVIm3qmVqqGOqeAVlTwbdnlniHHlWQw1bRDZU4kBMPJL9j4YJefUcvIsB' + 'd2acOEdmpPjTc8WAsC3D7YayVqWtaR23Kp/9E0PtjvGuXwlzfVsFQ+2OQRRCIYRCK' + 'IRCKIQQQgghhBBCCCGEEEIIIYQQQgghhBBCCCGEEEIIIYQQQqLI/0+Vt3dKyGP/AA' + 'AAAElFTkSuQmCC';
    }
  }]);
